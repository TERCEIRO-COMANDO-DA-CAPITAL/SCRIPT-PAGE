...
-- Configurações para o FOV Circle
local fovConfig = {
    Visivel = false,
    ColorMode = "RGB",
...
local cores = {
    ["Rojo de Cria"] = Color3.fromRGB(170, 0, 255),
    ["Azul Neon"] = Color3.fromRGB(0, 255, 255),
    ["Amarelo Perigo"] = Color3.fromRGB(255, 255, 0),
    ["Verde-Tóxico"] = Color3.fromRGB(0, 255, 0),
    ["RGB"] = "RGB"
}
...
local function hsvToRgb(h, s, v)
    local f = math.floor
    local p = (1 - s) * v
    local q = v * (1 - s * (h / 60 % 1))
    local t = v * (1 - s * (1 - (h / 60 % 1)))
    local i = f(h / 60) % 6
...
-- Variáveis para FOV GUI
local FOVGui
local ConexaoFOV

-- Função para garantir a existência da GUI do FOV
local function EnsureFOVGui()
    ...
    stroke.Transparency = fovConfig.Transparency
...
-- Função para atualizar o tamanho do FOV com base no aimbot ativo
local function UpdateFOVSize()
    if not FOVGui then return end
    local radius = 0
    if ConfiguracoesAimbotDiscreto.Ativado then
        radius = ConfiguracoesAimbotDiscreto.CampoDeVisao
    elseif ConfiguracoesAimbotAgressivo.Ativado then
        radius = ConfiguracoesAimbotAgressivo.CampoDeVisao
    end
    if radius > 0 then
        FOVGui.FOVCircle.Size = UDim2.new(0, radius * 2, 0, radius * 2)
    end
end

-- Função para alternar o FOV Circle
local function AlternarFOV(visivel)
    fovConfig.Visivel = visivel
    if visivel then
        EnsureFOVGui()
        UpdateFOVSize()
        ConexaoFOV = RunService.RenderStepped:Connect(function()
            if not fovConfig.Visivel then return end
            local stroke = FOVGui.FOVCircle.FOVStroke
            local alvo = ConfiguracoesAimbotDiscreto.AlvoAtual or ConfiguracoesAimbotAgressivo.AlvoAtual
            if alvo then
                stroke.Color = Color3.fromRGB(0, 255, 0) -- Verde quando lockado
            else
                if fovConfig.ColorMode == "RGB" then
                    local h = tick() * fovConfig.RainbowSpeed % 360
                    stroke.Color = hsvToRgb(h, 1, 1)
                else
                    stroke.Color = cores[fovConfig.ColorMode] or fovConfig.Color
                end
            end
        end)
    else
        if ConexaoFOV then
            ConexaoFOV:Disconnect()
            ConexaoFOV = nil
        end
        if FOVGui then
            FOVGui:Destroy()
...
local SecaoFOV = AbaESP:AddSection({"FOV Circle"})
AbaESP:AddToggle({
    Name = "Mostrar FOV",
...
AbaESP:AddDropdown({
    Name = "Cor do FOV",
    Description = "Selecione a cor ou modo",
    Options = {"Rojo de Cria", "Azul Neon", "Amarelo Perigo", "Verde-Tóxico", "RGB"},
    Default = "RGB",
    Callback = function(Valor)
        fovConfig.ColorMode = Valor
        if Valor ~= "RGB" then
            fovConfig.Color = cores[Valor]
        end
    end
})
...
local aviso = Window:MakeTab({"info", "star"})

aviso:AddDiscordInvite({
    Name = "DISCORD  -   CONVITE",
    Description = "Entrar no server",
    Logo = "rbxassetid://100971981026789",
    Invite = "https://discord.gg/auuzF3AErg",
})

local Section = aviso:AddSection({"Creditos"})
local Paragraph = aviso:AddParagraph({"DONOS", "[</>] dev - Menor DK\n[ ☯ ]Dono - RReze"})
local Paragraph = aviso:AddParagraph({"UPDATES", "-- Uptdates:\n[ + ] Adicionado aimbot com modo Discreto (mira suave) e Agressivo (mira rápida)\n[ + ] Adicionado ESP com visual moderno (nomes, vida, arma, contornos coloridos)\n[ + ] Adicionado destaque para o alvo do aimbot (contorno amarelo pulsante)\n[ + ] Adicionado exibição da arma equipada no ESP\n[ + ] Adicionado exibição das armas no inventário no ESP\n[ + ] Adicionado suporte a ícones de armas no ESP\n[ + ] Adicionado opção para ignorar alvos com escudo nos aimbots\n[ ~ ] Melhorado aimbot Discreto para mira mais precisa e suave\n[ ~ ] Melhorado aimbot Agressivo para mira mais rápida e estável\n[ ~ ] Melhorado ESP para maior fluidez em combates rápidos\n[ + ] Integrado FOV Circle configurável com cores customizáveis e modo RGB"})
